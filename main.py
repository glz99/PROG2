...
def form():
    ...
    if request.method.lower() == "post":
        ...
        #summe.json
        data = opendatei1()  # Datei öffnen von daten1.py
...
def auswertung():
    ...
    try:
        with open("summe.json") as open_file:             #summe.json wird geöffnet um nacher zu bearbeiten
...
def get_data():
    ...
    sorte = ["Glas", "Pet", "Karton"]    #Glas Pet und Karton als Sorte sind vordefiniert, die bleiben
    anzahl = []                          #für die Anzahl wird eine neue Liste geöffnet und die Anzahle dort rein gespeichert
    try:
    ...
    anzahl.append(data[0][0])       #zu der Anzahlliste wird jeweils der 1. eintrag aus dem ersten eintrag hinzugefügt
    anzahl.append(data[0][1])       #dann der zweite Eintrag aus dem ersten Eintrag
    anzahl.append(data[0][2])       #dann der dritte Eintrag aus dem ersten Eintrag

    return sorte, anzahl            #mit dem Return sorte und Anzahl wird dann unten weitergerechnet


def viz():
    sorte, anzahl = get_data()
    fig = px.bar(x=sorte, y=anzahl)    #hier wird definiert, welche Achse von welchen Daten gebraucht wird
    fig.update_layout(
        xaxis_title="Sorte",   #Achsentitel
        yaxis_title="Anzahl",
    )

    div = plot(fig, output_type="div")

    return div              #der Funktion viz() wird oben ein Name gegeben (div) diese wird dann returnt
